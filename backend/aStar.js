...
function aStar(graph, startNode, endNode) {
  ...
  fScore.set(startNode, haversine(startNodeData.lat, startNodeData.lon, endNodeData.lat, endNodeData.lon));
  ...
  while (!pq.isEmpty()) {
    ...
    for (let [neighbor, weight] of graph.getNeighbors(current)) {
      ...
      if (tentativeG < gScore.get(neighbor)) {
        ...
        const h = haversine(neighborNode.lat, neighborNode.lon, endNodeData.lat, endNodeData.lon);
